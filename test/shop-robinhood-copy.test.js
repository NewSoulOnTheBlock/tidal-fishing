...
test('shop and onboarding copy use Robinhood Chain ETH/$TIDAL labels, not Solana/SBF labels', () => {
  const copy = [SHOP_VISIBLE_COPY, TUTORIAL_COPY, BAIT_COPY].join('\n');
  assert.match(copy, /Pay with ETH|Buy ×\$\{qty\} bait · ETH/);
  assert.match(copy, /Pay with \$TIDAL|Buy bait with <b>ETH<\/b> or <b>\$TIDAL<\/b>/);
