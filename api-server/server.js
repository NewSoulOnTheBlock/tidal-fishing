...
// $SBF mint. The relaunched pump.fun mint is authoritative. A stale env value
...
const TIDE_MINT_NEW = 'HBibqRqqzAbnvZ4ogkcma6nzaoNWgpEimajVjHA3pump';
...
// calls). Keep env additions additive so a stale CORS_ORIGIN does not drop the
// production domains.
...
// mint withdrawable $SBF. Regenerate after editing fish data.
...
// Daily $SBF check-in bonus (credited to the withdrawable ledger). Kept small
...
function requireTrustedApiOrigin(req, res, next) {
  ...
  if (adminKeyValid(requestAdminKey(req))) return next();
...
// Express error handler for CORS rejections. Keep the response small and generic.
...
app.use(cors({
  origin(origin, callback) {
    // Browsers from our approved domains get CORS credentials. Requests with no
    // Origin receive no CORS headers and are rejected by requireTrustedApiOrigin
    // for every /api route except health/admin-secret calls.
...
// Security headers. CSP disabled (this is a JSON API). Cross-origin resource
// policy stays relaxed because CORS + requireTrustedApiOrigin now define exactly
// which frontend origins can read API responses.
app.use(helmet({
...
app.post('/api/withdraw', withdrawLimiter, async (req, res) => {
  try {
    ...
    if (reserve.rows.length === 0) {
      ...
      return res.status(400).json({ error: `Insufficient earned balance. You can withdraw up to ${avail} $SBF.`, withdrawable: avail });
    }
    ...
    try {
      ...
      try {
        ...
        throw new Error('Treasury has no $SBF token account');
      }
...
// It is now protected by the same trusted-origin gate as every other API route.
app.get('/api/widget', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      tagline: 'Cast a line, earn $SBF, climb the leaderboard.',
    });
  } catch (error) {
    console.error('[widget] Error:', error);
    res.json({ leader: 'Tidal anglers', earned: '0', players: 0, tagline: 'Cast a line and earn $SBF.' });
  }
});
